...
def effMasse(B):
    ...
    params1, covariance1 = curve_fit(f=linear, xdata=wavesquare, ydata=diff1)
    ...
    m1_eff = unp.sqrt(e**3*2.8e24*0.42/(8*np.pi**2*epsilon0*c**3*14.62e12*m0**2*n))
    ...
    params2, covariance2 = curve_fit(f=linear, xdata=wavesquare, ydata=diff2)
    errors2 = np.sqrt(np.diag(covariance2))
    print(' m = ', params2[0], ' +/- ', errors2[0], ' ')
    print(' b = ', params2[1], ' +/- ', errors2[1], ' ')
    a2 = ufloat(params2[0], errors2[0])*1e12  # 1e12, da wavesquare in µm²
    m2_eff = unp.sqrt(e**3*1.28e24*0.42/(8*np.pi**2*epsilon0*c**3*6.9e12*m0**2*n))
    ...
    # Plotte Messwerte und Regression
    waveplot = np.linspace(wavesquare[0], wavesquare[-1], 10)
